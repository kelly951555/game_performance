...
if history_resource is False:
    ...
    result = history_resource.find_all("div", {'class': 'number'})
    print('---遊戲紀錄計算中---')
    rows = list()
    result_len = int(len(result)/5)+1
    if result_len <= 0:
        print('No Data')
    else:
        for r in range(1, int(len(result) / 5) + 1):
            sn_path = '#history > div:nth-child({}) > div > table > tbody > tr:nth-child(10) > td:nth-child(1)'.format(
                r)
            player_path = '#history > div:nth-child({}) > div > table > tbody > tr:nth-child(4) > td'.format(r)
            room_path = '#history > div:nth-child({}) > div > table > tbody > tr:nth-child(8) > td:nth-child(3)'.format(
                r)
            coin_in_path = '#history > div:nth-child({}) > div > table > tbody > tr:nth-child(10) > td:nth-child(4)'.format(
                r)
            coin_out_path = '#history > div:nth-child({}) > div > table > tbody > tr:nth-child(12) > td:nth-child(2)'.format(
                r)
            # SN
            for sn in history_resource.select(sn_path):
                sn = sn.text.strip()
            # player
            for player in history_resource.select(player_path):
                player = player.text.strip().split(')')[1].replace(' ', '')
            # room
            for room in history_resource.select(room_path):
                room = room.text.strip()
            for coin_in in history_resource.select(coin_in_path):
                coin_in = float(
                    coin_in.text.replace('(', '').replace(')', '').replace(' ', '').replace('+', '').replace(',', ''))
            for coin_out in history_resource.select(coin_out_path):
                coin_out = float(
                    ...
                    if is_zero != '0':
                        ...
                        if player_performance is False:
                            ...
                            result2 = room_df.merge(df_performance, how='outer', indicator=True) \
                                .replace(['both', 'left_only', 'right_only'],
                                         ['PASS', 'Performance FAIL', 'History FAIL'])
                            result2.set_axis(
                                ['Room', 'Coin in', 'Coin out', 'Net Win', 'RTP', 'Avg. Bet', 'People', 'Game',
